...
import { Check, Loader2, Send, Sparkles, Briefcase, CreditCard, PiggyBank, Landmark, FileText, AlertTriangle } from 'lucide-react';
...
export default function OnboardingAlivia({ onDone }) {
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => {
            ...
            setMsgs([
                ...
                { role: 'alivia', text: 'Vou deixar sua conta pronta com você. A cada etapa eu **preparo o formulário** e você só preenche — assim nada fica faltando.' },
                { role: 'alivia', text: 'Pra começar, **como você quer que eu te chame?**' },
            ]);
            setPhase('nome');
        }, 1200);
            ...
            <div className={`relative w-full max-w-2xl h-full sm:h-[94vh] sm:max-h-[840px] flex flex-col sm:rounded-3xl border shadow-2xl overflow-hidden ${isDark ? 'bg-[#0e0f12] border-white/10' : 'bg-white border-slate-100'}`}>
                ...
                <div className={`flex items-center gap-3.5 px-6 py-5 border-b ${isDark ? 'border-white/10 bg-white/[0.02]' : 'border-slate-100 bg-slate-50'}`}>
                    ...
                    <div className="min-w-0 flex-1">
                        <p className={`text-lg font-black flex items-center gap-1.5 ${isDark ? 'text-white' : 'text-slate-800'}`}>Bem-vindo(a) à Alívia <Sparkles className="w-5 h-5 text-emerald-500" /></p>
                        <p className="text-[13px] text-emerald-500 font-semibold flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> Vamos configurar sua conta</p>
                    </div>
                    {phase !== 'done' && <button onClick={pular} className={`text-[13px] font-bold px-3 py-2 rounded-lg transition ${isDark ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-100'}`}>Pular</button>}
                </div>
                    ...
                    {!typing && (
                        <div className="pt-1">
                            ...
                            {phase === 'patrimonio' && <ChoiceRow isDark={isDark} options={[{ label: 'Tenho sim', icon: Landmark, value: true }, { label: 'Ainda não', value: false }]}
                                onPick={(sim) => sim ? advance('Tenho investimentos', 'Show! **Preparei o cadastro do investimento** — preencha os dados do que você já tem 👇', 'ativoIntro') : advance('Ainda não', SALDO_MSG, 'saldo')} />}
                            ...
                            {phase === 'ativoMore' && <ChoiceRow isDark={isDark} options={[{ label: 'Adicionar outro', value: true }, { label: 'Concluir', value: false }]}
                                onPick={(sim) => sim ? setActiveForm('ativo') : advance('Investimentos ok', SALDO_MSG, 'saldo')} />}
    ...
    function onSaldo(v) {
        ...
        advance(`R$ ${money(saldo)} na conta`, null, 'finish');
...
function Bubble({ isDark, role, text }) {
    ...
    return (
        <div className={`flex gap-2.5 max-w-[90%] ${me ? 'ml-auto flex-row-reverse' : ''}`}>
            {!me && <img src={aliviaAvatar} alt="" className="w-8 h-8 rounded-full object-cover shrink-0 mt-1" />}
            <div className={`px-4 py-3 rounded-2xl text-[15px] leading-relaxed ${me ? 'bg-emerald-500 text-white rounded-tr-md' : (isDark ? 'bg-white/[0.05] text-slate-200 rounded-tl-md' : 'bg-slate-100 text-slate-700 rounded-tl-md')}`}
...
const inputBase = (isDark) => `px-4 py-3.5 rounded-xl border text-base font-semibold outline-none transition ${isDark ? 'bg-white/5 border-white/10 text-white placeholder-slate-500 focus:border-emerald-500' : 'bg-white border-slate-200 text-slate-800 placeholder-slate-400 focus:border-emerald-500'}`;
function SendBtn({ onClick, disabled }) {
    return <button onClick={onClick} disabled={disabled} className="shrink-0 w-12 h-12 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white flex items-center justify-center transition disabled:opacity-40"><Send className="w-5 h-5" /></button>;
}
...
function ChoiceRow({ isDark, options, onPick }) {
    return (
        <div className="flex gap-2 flex-wrap">
            {options.map((o, i) => {
                const Icon = o.icon;
                return <button key={i} onClick={() => onPick(o.value)} className={`flex items-center gap-2 px-5 py-3 rounded-xl border text-[15px] font-bold transition active:scale-95 ${isDark ? 'border-white/10 bg-white/[0.02] text-slate-200 hover:border-emerald-500/40 hover:bg-emerald-500/[0.06]' : 'border-slate-200 bg-white text-slate-700 hover:border-emerald-300 hover:bg-emerald-50'}`}>{Icon && <Icon className="w-5 h-5 text-emerald-500" />} {o.label}</button>;
            })}
        </div>
    );
}
function OpenFormBtn({ isDark, label, icon: Icon, onClick, secondary }) {
    return (
        <div className="flex gap-2 flex-wrap">
            <button onClick={onClick} className="flex items-center gap-2 px-5 py-3.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white text-[15px] font-black transition active:scale-95 shadow-md shadow-emerald-500/30">
                {Icon && <Icon className="w-5 h-5" />} {label}
            </button>
            {secondary && <button onClick={secondary.onClick} className={`px-5 py-3.5 rounded-xl border text-[15px] font-bold transition ${isDark ? 'border-white/10 text-slate-300 hover:bg-white/5' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{secondary.label}</button>}
        </div>
    );
}
