    ...
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="pat-card p-4">
        <div className="flex items-center justify-between mb-3">
          <p className={`text-xs font-black ${isDark ? 'text-white' : 'text-slate-800'}`}>Cadastrados</p>
        ...
        {sorted.length === 0 ? (
          <div className="text-center py-10">
            ...
            <p className="text-[11px] text-slate-500 mt-1">
              {isCadastro
                ? 'Cadastre seu salário para não precisar lançar todo mês.'
                : 'Cadastre em Cadastros › Recebimentos Fixos para confirmar por aqui.'}
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {sorted.map(inc => {
              const received = isReceived(inc);
              return (
                <div key={inc.id} className={`flex items-center justify-between gap-3 p-3 rounded-xl border ${isDark ? 'border-white/[0.06] bg-white/[0.02]' : 'border-slate-100 bg-slate-50'}`}>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className={`text-sm font-black truncate ${isDark ? 'text-white' : 'text-slate-800'}`}>{inc.name}</p>
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${received ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>
                        {received ? 'Recebido' : 'A receber'}
                      </span>
                    </div>
                    <p className="text-[10px] font-bold text-slate-500 mt-0.5 flex items-center gap-1.5">
                      <Calendar className="w-3 h-3" /> Dia {inc.day || 1} · {catLabel(inc.category)}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-sm font-black tabular-nums text-emerald-500">R$ {fmt(inc.value)}</span>
                    {/* Só LANÇAMENTOS confirma (movimenta dinheiro) */}
                    {!isCadastro && !received && (
                      <button
                        onClick={() => { setConfirming(inc); setActualValue(String(inc.value ?? '')); }}
                        title="Confirmar recebimento deste mês"
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest bg-emerald-500 hover:bg-emerald-400 text-white transition-all"
                      >
                        <CheckCircle2 className="w-3.5 h-3.5" /> Confirmar
                      </button>
                    )}
                    {/* Só CADASTROS edita/exclui */}
                    {isCadastro && (
                      <>
