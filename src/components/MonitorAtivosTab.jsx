...
export default function MonitorAtivosTab() {
  ...
  const toDisplay = (value, nativeCur) => {
    ...
    return value; // outras moedas: mantém
  };
  ...
  const fmtPrice = (v) => {
    ...
    const dec = abs > 0 && abs < 1 ? 4 : 2; // cripto de baixo valor mostra mais casas
  ...
  // Agrupa a watchlist por grupo, na ordem definida em GROUPS.
    ...
    <div className="space-y-5 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className={`text-sm mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Acompanhe em tempo real o preço dos ativos que você segue</p>
        </div>
      ...
      {/* Barra de status (última atualização) */}
