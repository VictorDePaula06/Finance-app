...
const AnalysisTab = ({ transactions = [], cards = [], subscriptions = [], fixedExpenses = [], manualConfig = {}, onUpdateConfig, initialView = 'relatorios' }) => {
  ...
  return (
    <div className="max-w-full overflow-x-hidden px-5 md:px-8 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 pt-6">
      {(view === 'relatorios' || view === 'periodo' || view === 'comparativo') && (
        <ReportsHub transactions={transactions} cards={cards} subscriptions={subscriptions} fixedExpenses={fixedExpenses} theme={theme} />
      )}
      {view === 'metas' && (
