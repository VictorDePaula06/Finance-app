    ...
    return (
        ...
        <div className={`rounded-2xl border overflow-hidden ${isDark ? 'border-white/10 bg-white/[0.02]' : 'border-slate-200 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)]'}`}>
            ...
            <div className={`flex items-center justify-between gap-3 flex-wrap px-4 sm:px-5 py-4 border-b ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
                <div className="min-w-0">
                    <h2 className={`text-[15px] font-black tracking-tight flex items-center gap-2.5 ${isDark ? 'text-white' : 'text-slate-800'}`}>
                        ...
                        {t('reg.recurringTitle')}
                    </h2>
                    <p className={`text-[12px] mt-1 ${muted}`}>{t('reg.recurringDesc')}</p>
                </div>
            ...
            <div className={`flex items-center gap-1 px-4 sm:px-5 py-2.5 border-b ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
                {[['all', t('common.all')], ['income', t('common.incomes')], ['expense', t('common.expenses')]].map(([id, label]) => {
                    const on = filter === id;
                    return (
            ...
            {rows.length === 0 ? (
                <div className="py-12 text-center px-4">
                    ...
                    <p className={`text-sm font-bold ${cell}`}>{t('reg.noneYet')}</p>
                    <p className={`text-xs mt-1 ${muted}`}>{t('reg.noneYetDesc')}</p>
                </div>
            ) : (
                    ...
                    <table className="w-full text-[13px]">
                        <thead>
                            <tr className={isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}>
                                ...
                                <th className={`${th} hidden sm:table-cell`}>{t('common.category')}</th>
                                <th className={`${th} text-right`}>{t('common.value')}</th>
                                <th className={`${th} hidden md:table-cell`}>{t('reg.dueDayShort')}</th>
                                <th className={`${th} hidden md:table-cell`}>{t('reg.payment')}</th>
                            ...
                            {rows.map(r => {
                                ...
                                return (
                                    <tr key={r.id} className={`transition-colors ${isDark ? 'hover:bg-white/[0.02]' : 'hover:bg-slate-50/70'}`}>
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-2.5 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className={`text-[11px] ${muted}`}>{r.isVariable ? t('common.variable') : t('common.fixed')}<span className="sm:hidden"> · {c.label}</span></p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-full ${income ? 'bg-emerald-500/12 text-emerald-500' : 'bg-rose-500/12 text-rose-500'}`}>
                                                {income ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />} {income ? t('common.income') : t('common.expense')}
                                            </span>
                                        </td>
                                        ...
                                        <td className={`px-4 py-3 hidden md:table-cell whitespace-nowrap ${cell}`}>{t('reg.dueDayShort')} {r.day || 1}</td>
                                        <td className={`px-4 py-3 hidden md:table-cell whitespace-nowrap ${cell}`}>{pay}</td>
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-end gap-0.5">
                                                <button onClick={() => setForm({ editing: r })} title={t('common.edit')}
                                                ...
                                                <button onClick={() => setDel(r)} title={t('common.delete')}
...
function CadastroForm({ isDark, uid, editing, cards, onClose }) {
    ...
    const submit = async (e) => {
        e.preventDefault();
        setError('');
        if (!name.trim() || !numBR(value)) { setError(t('reg.errFill')); return; }
        ...
        try {
            if (editing) { await updateDoc(doc(db, cfg.collection, editing.id), data); toast.success(t('reg.okUpdated')); }
            else { await addDoc(collection(db, cfg.collection), { ...data, userId: uid, createdAt: Date.now() }); toast.success(income ? t('reg.okCreatedIncome') : t('reg.okCreatedExpense')); }
            onClose();
        } catch (err) { console.error(err); setError(t('recf.errSave')); setSaving(false); }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className={`relative w-full max-w-md rounded-3xl border shadow-2xl p-6 max-h-[92vh] overflow-y-auto ${isDark ? 'bg-[#141518] border-white/10' : 'bg-white border-slate-100'}`}>
                ...
                <form onSubmit={submit} className="space-y-3.5">
                            ...
                            {['income', 'expense'].map(k => {
                                ...
                                return (
                                    <button key={k} type="button" disabled={!!editing} onClick={() => changeKind(k)}
                                        ...
                                        <Icon className="w-4 h-4" /> {inc ? t('common.income') : t('common.expense')}
                                    </button>
                                );
                            })}
                        </div>
                    ...
                    <Field label={t('common.name')}><input value={name} onChange={e => setName(e.target.value)} placeholder={income ? t('reg.nameIncomePh') : t('reg.nameExpensePh')} className={inputCls} maxLength={50} autoFocus /></Field>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <Field label={income ? t('reg.receivesDay') : t('reg.dueDay')}><input inputMode="numeric" value={day} onChange={e => setDay(e.target.value.replace(/\D/g, '').slice(0, 2))} placeholder="5" className={inputCls} /></Field>
                    </div>

                    <Field label={t('reg.fixedOrVariable')}>
                        <div className={`grid grid-cols-2 gap-1 p-1 rounded-xl border ${isDark ? 'border-white/10 bg-white/[0.03]' : 'border-slate-200 bg-slate-100/70'}`}>
                            {[[false, t('common.fixed'), t('reg.fixedHint')], [true, t('common.variable'), income ? t('reg.varHintIncome') : t('reg.varHintExpense')]].map(([v, label, hint]) => {
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1">
                        <button type="button" onClick={onClose} className={`py-3 rounded-xl font-bold text-sm border transition ${isDark ? 'border-white/10 text-slate-300 hover:bg-white/5' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{t('common.cancel')}</button>
