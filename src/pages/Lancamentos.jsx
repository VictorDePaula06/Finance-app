        ...
        <div className="max-w-6xl mx-auto w-full">
            ...
            <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                        <h1 className={`text-2xl sm:text-3xl font-black tracking-tight truncate ${isDark ? 'text-white' : 'text-slate-800'}`}>{t('stm.title')}</h1>
                        <p className={`text-[13px] sm:text-sm mt-0.5 ${muted}`}>{t('stm.subtitle')}</p>
                    </div>
                </div>
                {/* Extrato é só consulta: lançar acontece em Contas a receber / Contas a pagar. */}
            </div>
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap mt-8 mb-3">
                <h2 className={`text-[15px] font-black tracking-tight flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>
                    ...
                    <span className={`text-[11px] font-semibold normal-case ${muted}`}>· {t('stm.readOnly')}</span>
                </h2>
...
export function LancamentoForm({ isDark, uid, kind, editing, saldoConta = 0, onClose, inline = false }) {
